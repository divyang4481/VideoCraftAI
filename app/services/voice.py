...
# `none` is the no-dubbing flag used in PR #981. This value is compatible with this value in the short term to avoid
# API users who have manually called this branch will be invalid immediately after upgrade; WebUI and new code will be used uniformly
# More explicit `no-voice`.
...
def mktimestamp(time_unit: float) -> str:
    ...
    edge_tts 7.x  `mktimestamp`, 
     Azure v2, Gemini, SiliconFlow 
    , . 
    """
...
def get_siliconflow_voices() -> list[str]:
    ...
    """
    # Silicon-based flowing list of sounds and corresponding genders (for display)
    voices_with_gender = [
...
def get_gemini_voices() -> list[str]:
    ...
    Returns:
        ,  ["gemini:Zephyr-Bright", "gemini:Puck-Upbeat", ...]
    """
...
def estimate_no_voice_duration(text: str) -> float:
    ...
    for char in normalized_text:
        # Unicode categories start with L to represent letters in various languages, and N to represent numbers. Already alone before
        # CJK and ASCII words are counted, and only the remaining text is counted here to avoid repeated counting of English.
...
def tts(
    ...
    if is_azure_v2_voice(voice_name):
        ...
        # Format: gemini:voice-Style; also continues to be compatible with the old gemini:voice-Gender.
        ...
        # Extract voice name from voice_name
        # Format: mimo:voice-Gender; if the caller has executed parse_voice_name,
        # then it might be mimo:voice. Both formats are compatible.
        parts = voice_name.split(":")
        if len(parts) >= 2:
        ...
        # Format: chatterbox:<voice>, voice can have -Female/-Male suffix for display
...
def convert_rate_to_percent(rate: float) -> str:
    ...
    # API or batch calls may pass in 0, 0.0, None, or an unconvertible null value; these values do not represent
    # Legal speaking speed, directly calculated will become -100% or an exception will be thrown. Here, we return to the normal speaking speed.
    # Avoid generating extremely slow audio or having the TTS process fail on boundary inputs.
    try:
        rate = float(rate)
...
def ensure_legacy_submaker_fields(sub_maker: SubMaker) -> SubMaker:
    ...
    edge_tts 7.x  `SubMaker`  `cues/get_srt()`,  Azure v2, 
    Gemini, SiliconFlow  `subs/offset`. , 
     edge_tts  edge . 
    """
...
def populate_legacy_submaker_with_full_text(
    ...
    : 
    1. edge_tts 7.x  `SubMaker`  `create_sub()`; 
    2.  Gemini, SiliconFlow  edge 
        `subs/offset` , ; 
    3.  TTS , , 
    ...
    # Clear old values to avoid dirty data overlay when the caller reuses objects.
    sub_maker.subs = []
    sub_maker.offset = []
    ...
    # When paths such as Gemini/SiliconFlow cannot obtain word-by-word boundaries, try to still use the project
    # The original strategy of "break sentences according to punctuation + allocate duration according to the number of characters". This will allow
    # create_subtitle() matches script segments and avoids rolling back to Whisper again.
    ...
    for index, sentence in enumerate(sentences):
        ...
        # The duration of the previous sentences is allocated in proportion to the number of characters, and the last sentence eats up the remaining duration.
        # Avoid integer rounding causing the total duration to be lost or the subtitle end time to be shorter than the audio.
...
def create_edge_tts_communicate(
    ...
    : 
    1.  edge_tts 7.x,  `boundary` ; 
    2.  Windows ,  edge_tts; 
    3.  `Communicate.__init__()`  `boundary`, 
       `unexpected keyword argument 'boundary'`,  TTS . 
...
def stream_edge_tts_chunks(
    ...
    # Here we explicitly create an independent event loop instead of reusing the external context. The purpose is to avoid
    # In the synchronization call stack, I encountered the problem of "the current thread does not have an event loop" or the problem of reusing loops across threads.
...
def azure_tts_v1(
    ...
    for i in range(3):
        try:
            ...
            # This is compatible with edge_tts 7.x and the old dependencies that may remain in the old version of the portable package:
            # 1. The new version supports `boundary` + `stream_sync()`
            # 2. The old version does not support `boundary` and usually only exposes asynchronous `stream()`
            ...
            with open(voice_file, "wb") as file:
                def _handle_chunk(chunk):
                    ...
                    if chunk_type == "audio":
                        ...
                        # Whether it's a synchronous stream from 7.x or a legacy asynchronous stream, as long as the event structure
                        # If there is still boundary information in it, it will be uniformly fed to SubMaker to ensure subsequent subtitle links.
                        # Still follow the existing logic of the project.
            ...
            # If TTS streaming writing times out or the network is abnormal before the first packet, 0-byte audio files will be left.
            # Such files are neither playable nor may mislead subsequent troubleshooting, so only empty files are cleared after failure;
            # If part of the data has been written, the on-site file is retained to facilitate analysis of the content returned by the server.
...
def siliconflow_tts(
    ...
    # Convert voice_volume to gain range for silicon flow
    # The default voice_volume is 1.0, and the corresponding gain is 0
    gain = voice_volume - 1.0
    # Make sure gain is in the range [-10, 10]
    ...
    for i in range(3):  # 3
        try:
            ...
            if response.status_code == 200:
                ...
                # The original subtitle structure of the project is still used here, so the old fields need to be filled in.
                sub_maker = ensure_legacy_submaker_fields(SubMaker())
                ...
                try:
                    ...
                    # Use text segmentation to create more accurate subtitles
                    # Split text into sentences based on punctuation
                    sentences = utils.split_string_by_punctuations(text)

                    if sentences:
                        # Calculate the approximate length of each sentence (distributed in proportion to the number of characters)
                        ...
                        for sentence in sentences:
                            ...
                            # Calculate the duration of the current sentence
...
def azure_tts_v2(
    ...
    for i in range(3):
        try:
            ...
            # speak_text_async() does not support the speaking speed parameter. After using SSML prosody, audition and
            # Formal generation will adjust the speech rate according to the voice_rate passed in from WebUI/API.
...
def gemini_tts(
    ...
    try:
        ...
        # google-genai uses the unified client to call text and TTS models. The context manager ensures
        # Release the HTTP connection after the request is completed, while retaining the original PCM transcoding and subtitle timeline logic.
        ...
        # The audio data is already raw bytes and does not require base64 decoding
        if isinstance(audio_data, str):
            # If it is a string, base64 decoding is required
            audio_bytes = base64.b64decode(audio_data)
        else:
            # If it is already a byte, use it directly
            audio_bytes = audio_data
        
        # Try different audio formats - Gemini may return different formats
        audio_segment = None
        
        # Gemini returns Linear PCM format, parsed according to document parameters
        try:
            audio_segment = AudioSegment.from_file(
                io.BytesIO(audio_bytes), 
                format="raw",
                frame_rate=24000,  # Gemini TTS
                channels=1,        # 
        ...
        # APIs, CLIs, or tests can directly target nested directories that don't yet exist as output locations. here at
        # Create the parent directory before actually writing the file to avoid a successful Gemini request ending up with
        # Losing results when the local path does not exist also makes this provider behave consistently with other TTS implementations.
        ensure_file_path_exists(voice_file)

        # pydub returns the open output file object. If it is not actively closed during batch generation, the file descriptor
        # It continues to accumulate and increases the chance of subsequent failure to overwrite or delete audio files on Windows.
        exported_audio = audio_segment.export(voice_file, format="mp3")
        exported_audio.close()
        
        logger.info(f"completed, output file: {voice_file}")
        
        # Gemini cannot get word-by-word boundary events like edge_tts, so we fall back to
        # The original `subs/offset` compatibility structure of the project ensures at least subsequent subtitles and duration.
        # The compute link can continue to work.
        sub_maker = ensure_legacy_submaker_fields(SubMaker())
...
def get_minimax_tts_endpoint() -> str:
    """
     ...
     TTS Key  TTS ;  MiniMax LLM Key , 
     LLM Base URL ,  Key  401. 
    """
...
def get_minimax_voice_catalog(
    ...
    """
     ...
     voice_id, voice_name, voice_type , 
    MiniMax . ,  WebUI, 
    API  CLI , . 
    """
...
def chatterbox_tts(
    ...
    payload = {
        ...
        "voice": voice,
        "response_format": "mp3",
        # OpenAI speech API accepts speed 0.25-4.0; VideoCraft AI's rate is a
...
# Arabic diacritics and Tatweel elongators may appear in edge-tts return text,
# These characters do not affect semantics, but can cause exact matches between script text and subtitle cue strings to fail.
...
def _match_script_line(script_lines: list[str], current_text: str, sub_index: int) -> str:
    ...
    # The final layer of Arabic tolerance: letterforms, diacritics or Tatweel returned by edge-tts
    # Might be different from the script. Normalized comparisons are only performed after regular matching fails, non-Arabic text is not affected.
...
def _build_subtitle_items_from_edge_cues(
    ...
    """
     edge_tts 7.x  `cues`  SRT . 

    : 
    edge_tts 7.x  `SubMaker.get_srt()` /. 
    , , 
    " /  /  /  / " . 
...
def _get_audio_duration_from_submaker(sub_maker: SubMaker):
    ...
    Get audio duration
    """
    # Priority is given to compatibility with the cues structure of edge_tts 7.x;
    # If it is an old structure manually filled by other TTS in the project, continue to read the offset.
    if hasattr(sub_maker, "cues") and sub_maker.cues:
...
if __name__ == "__main__":
    ...
    async def _do():
        ...
        text = """
        2023
1476.94, 19.01%, 747.34, 19.16%. EPS59.49. , 444.25, 20.26%, 31.86%; 218.58, 19.33%, 29.37%. 
, . 
2023Q4
, ; ; 27%, . 
