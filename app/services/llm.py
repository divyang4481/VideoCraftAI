...
def _normalize_text_response(content, llm_provider: str) -> str:
    # Different LLM SDKs may return None or empty strings in abnormal or intercepted scenarios.
    # Even non-string objects are returned. A comprehensive check is performed here to avoid subsequent direct calls.
    # Property errors like `NoneType` are thrown when `.replace()` is used.
    if content is None:
    ...
    # Reasoning models such as MiniMax M3 and DeepSeek R1 may wrap internal reasoning in
    # Returned in `<think>...</think>`. Video scripts and keywords only require final readable text,
    # If the service layer is not cleaned up uniformly, WebUI, subtitles and dubbing will treat the thinking process as text.
    content = _THINK_BLOCK_RE.sub("", content)
    content = _UNCLOSED_THINK_BLOCK_RE.sub("", content).strip()
    if not content:
        raise ValueError(f"[{llm_provider}] returned empty text content")

    # The previous ``strip()`` has cleared the leading and trailing whitespace. Here you must retain the single line breaks and
    # Double line breaks: Script generation relies on double line breaks to distinguish paragraphs, and subtitle processing will also read user copy line by line.
    return content


def _sanitize_error_message(error: object) -> str:
    """
    Cleanup returns to WebUI/API error message to avoid customizing base_url Credentials leaked in .

    Some OpenAI-compatible SDK will request URL Enter the exception information as it is. If the user wants to
    Proxy gateway configured `https://user:pass@example.com/v1`, return directly `str(e)`
    will expose the password to the page,API Caller or subsequent logs. Only the error copy is processed here and is not changed.
    Actual request address to avoid affecting the normal calling link.
    """
    message = str(error)
...
def _extract_chat_completion_text(response, llm_provider: str) -> str:
    # The OpenAI compatible interface may return no choices or
    # Or choices/message/content is an empty response object.
    # Structure verification is performed uniformly here to avoid `NoneType is not subscriptable`.
    # This type of underlying property access error.
...
def _get_response_field(value, key: str):
    """compatible dict and SDK Fields of the response object are read."""
...
def _extract_qwen_generation_text(response) -> str:
    """
    from DashScope Generation Extract text from the response.

    Qwen use `messages` What is returned when calling is chat structure:
    `output.choices[0].message.content`;old completion form will return
    `output.text`. Both paths here are compatible, avoid `output.text` for None hour
    continue `.replace()` Trigger undiagnosable AttributeError. 
    """
...
def _generate_response(prompt: str, app_config=None) -> str:
    try:
        # WebUI allows users to prepare the next piece of copy during video generation. The caller can pass in the commit instant
        # Configuration snapshot to ensure that the background task will not end and new configurations will be applied during model request retries.
        # Instead switch to another Provider, Base URL or model.
        ...
        # Ollama's default address depends on whether it is currently running in a container and cannot be used as a static registry.
        # Values ​​are saved; the Registry is still responsible for the model and required rules, and the runtime environment differences are explained here.
        ...
        if adapter == "gemini":
            ...
            try:
                # The new version of google-genai exposes model services through the unified Client. context manager
                # The underlying HTTP connection will be closed after the request is completed to avoid accumulation of connection resources during frequent generation.
                with genai.Client(
                    api_key=api_key,
        ...
        if adapter == "cloudflare_ai_gateway":
            ...
            # Cloudflare's current recommended AI Gateway REST API is compatible with the OpenAI SDK.
            # Account ID is used to construct a unified endpoint, and Gateway ID is selected through the request header; here
            # Workers AI's /ai/run/{model} dedicated interface is no longer called.
            client = OpenAI(
                api_key=api_key,
                base_url=(
        ...
        if adapter == "azure":
            # Azure OpenAI SDK uses `azure_endpoint` and `api_version` to generate private request addresses,
            # You cannot continue to reuse the common OpenAI-compatible `base_url` initialization logic below.
            # Here, the request is completed within the Azure branch and returned immediately to avoid subsequent fallback on the client.
            # Override, causing the user's configured Azure credentials to pass validation but not be used for the actual request.
...
def test_connection() -> tuple[bool, str, float]:
    """
    Use current Provider Configure to initiate a minimum request to verify whether the actual generated link is available.

    Direct reuse of connection tests `_generate_response()`, thus covering API Key, Base URL, 
    model name and Provider Private field, but will not enter the script-generated retry logic and will not be sent
    The user's video theme or copy. The return value is success status, error information and request time.
    """
...
def _limit_script_text(text: str | None, max_length: int, field_name: str) -> str:
    ...
    # The API layer has used Pydantic for length verification; we continue to cover it here for protection.
    # When WebUI or internal services directly call generate_script, the long prompt words will not be sent to the model.
    # Avoid token cost exceptions and request failures.
    logger.warning(
...
def _normalize_script_paragraph_number(paragraph_number: int | None) -> int:
    ...
    if value < MIN_SCRIPT_PARAGRAPH_NUMBER or value > MAX_SCRIPT_PARAGRAPH_NUMBER:
        # Both WebUI and API will limit the scope; internal calls are handled here to avoid direct expansion of abnormal parameters.
...
def build_script_prompt(
    ...
    # Splice "script generation rules" and "runtime context" separately. This allows advanced users to override the default
    # system prompt, and will not miss the parameters such as video theme, language, and number of paragraphs that must be included in each generation.
...
def generate_script(
    ...
    for i in range(_max_retries):
        try:
            ...
            if final_script and "The quota for the day has been used up" in final_script:
...
def generate_terms(
    ...
    if match_script_order:
        ...
        # In ordered keyword mode, the number of examples must be consistent with amount to avoid the model being fixed.
        # The 4 examples are misleading, resulting in long copy only returning a small number of keywords, affecting material coverage.
        example_terms = [
...
""".strip()
    ...
    for i in range(_max_retries):
        try:
            ...
            if response.startswith("Error: "):
                # The public return type of generate_terms is List[str]. If you put Provider
                # The error text is returned as it is. When the downstream only performs null value judgment, non-empty strings will be mistaken for success.
                # The material download loop also iterates over the wrong copy character by character, generating meaningless external requests.
                # Here, an empty list is uniformly returned, allowing the task orchestration layer to immediately end the task at the actual fault location.
            ...
            if response:
                ...
                if match:
                    try:
                        ...
                        # The retry process is retained here, but the non-standard JSON returned by LLM must be logged.
                        # Otherwise, subsequent troubleshooting will not be able to locate if the search term is empty.
                        # Is it a model format problem or a parsing logic problem?
                        logger.warning(f"failed to generate video terms: {str(e)}")
...
#
# Generate titles, captions and hashtags commonly used when publishing to short video platforms based on video themes and scripts.
# This capability only reuses the existing LLM provider, does not connect to any external publishing services, and does not affect the main video generation link.
# =============================================================================

# Different platforms have different preferences for copy length and number of hashtags. A conservative upper limit is used here to avoid model returns
# If the content is too long, the caller will need to crop it twice.
SOCIAL_PLATFORMS = {
...
# Universal catch-all label when LLM is unavailable. This is intentionally not bound to a certain country or language to ensure that the API
# Available structures can be returned for different scenarios such as Chinese, English, and Vietnamese.
...
def _limit_social_text(text: str | None, max_length: int, field_name: str) -> str:
    ...
    # The API layer will limit the length; continuing to cover it here is to protect internal calls or future WebUI
    # When calling directly, overlong content will not be sent to the model to avoid token cost anomalies.
    logger.warning(
        f"{field_name} is too long and will be truncated to {max_length} characters."
    )
    return value[:max_length]
...
def _normalize_hashtags(raw, count: int) -> List[str]:
    """
    Will LLM returned hashtag unified into `#tag` Format.

    LLM May return strings, arrays, phrases with spaces, repeated tags, or content containing punctuation.
    Centralized cleaning here can stabilize the interface response structure and avoid empty tags and
    Duplicate tags or tags that don't follow common formats hashtag. 
    """
    if isinstance(raw, str):
        candidates = re.split(r"[\s,]+", raw)
    elif isinstance(raw, (list, tuple)):
        # Each item in the array is considered a complete label, so "du lich" becomes
        # "#dulich" instead of splitting into two tags.
...
def _parse_social_metadata(response: str, platform: str) -> dict:
    ...
    try:
        ...
        # Some models will wrap a description text or markdown fence in JSON.
        # The API caller only needs the stable structure, so here we try to extract the first JSON object.
...
def _fallback_social_metadata(
    ...
    if not title and script:
        # When there is no theme, use the first sentence of the script to generate the title to avoid the interface returning an empty title.
...
def generate_social_metadata(
    ...
    """
    Generate short video publishing copywriting metadata.

    The return structure is fixed to `{"title": str, "caption": str, "hashtags": List[str]}`. 
    if LLM If it is unavailable or the returned format is abnormal, it will be downgraded to a general heuristic result, ensuring API
    The caller always gets a data structure that can be displayed and edited before publishing.
    """
    platform = _resolve_social_platform(platform)
    language = _normalize_social_language(language)
    video_subject = _limit_social_text(
