...
class _StreamingResponse:
    """supply requests.Response exist Sonilo The smallest interface actually used in the service."""
...
class TestSoniloService(unittest.TestCase):
    ...
    def test_request_timeout_clamps_fractional_and_invalid_values(self):
        """The read timeout must remain Requests Accepts a positive integer and limits the maximum wait time."""
        test_cases = [
            (0.5, (15, 1)),
            (1.1, (15, 2)),
            (1800.5, (15, 1800)),
    ...
    def test_connection_rejects_malformed_service_lists(self):
        """200 Response to missing canonical service list cannot be made to WebUI Report successful connection."""
        invalid_payloads = [
            {},
    ...
    def test_connection_converts_network_and_invalid_json_errors(self):
        """Connectivity testing for network outages and non- JSON Responses are converted to stable domain exceptions."""
        with (
            patch.object(sonilo.config, "app", {"sonilo_api_key": "test-key"}),
            patch.object(
                sonilo.requests,
    ...
    def test_create_video_proxy_cleans_file_after_execution_failures(self):
        """FFmpeg Hidden proxy files cannot be left behind when timed out, not executable, or when encoding fails."""
        failure_cases = [
            (
    ...
    def test_create_video_proxy_rejects_empty_and_oversized_outputs(self):
        """FFmpeg If the return is successful, it must also be verified again that the proxy file exists, is not empty, and does not exceed the upper limit."""
    ...
    def test_request_bgm_preserves_existing_output_and_cleans_temp_on_failures(self):
        """HTTP, stream reading and audio verification failures cannot overwrite existing results or leave half-finished products."""
        audio_event = _event(
            "audio_chunk",
            stream_index=0,
    ...
    def test_generate_bgm_converts_file_errors_and_cleans_proxy(self):
        """File system failures must also be converted into degradable exceptions and the generated agents must be cleaned up."""
        with tempfile.TemporaryDirectory() as temp_dir:
            source = Path(temp_dir) / "source.mp4"
            proxy = Path(temp_dir) / "proxy.mp4"
            source.write_bytes(b"video")
    ...
    def test_generate_bgm_rejects_missing_key_and_input_before_proxy_work(self):
        """Should fail quickly when credentials or input files are missing and cannot be called FFmpeg or external API. """
        with tempfile.TemporaryDirectory() as temp_dir:
            source = Path(temp_dir) / "source.mp4"
            source.write_bytes(b"video")
            with (
