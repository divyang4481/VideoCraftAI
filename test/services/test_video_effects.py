...
class TestZoomTransitions(unittest.TestCase):
    ...
    def test_zoomin_preserves_geometry_and_zooms_over_time(self):
        ...
        # Magnification starts at 1x, so the first frame should be consistent with the original.
        np.testing.assert_allclose(first, original, atol=2)
        # The last frame is from a center cropped and enlarged area, and should be significantly different from the original image.
    ...
    def test_zoomout_starts_zoomed_and_returns_to_source(self):
        ...
        # The first frame of the reduction is 1.2x, and the end is exactly back to the original ratio.
    ...
    def test_zoom_frame_responds_to_subpixel_scale_changes(self):
        ...
        # These two ratios would fall into the same crop size in the old integer crop algorithm, producing exactly the same frame,
        # This is followed by a sudden jump when crossing an integer boundary. Subpixel sampling should be able to respond to such small scale changes.
    ...
    def test_zoom_frame_keeps_center_stable_for_odd_resolution(self):
        ...
        # Odd width and height have only one exact center pixel, which should not drift horizontally or vertically after scaling.
        np.testing.assert_allclose(
