#!/usr/bin/env python3
"""Cross-platform installation and video generation for the VideoCraft AI Skill."""
...
PROJECT_ARCHIVE_URL = (
    "https://github.com/divyang4481/VideoCraftAI/archive/refs/heads/main.zip"
)
DEFAULT_ROOT = Path.home() / "VideoCraft AI"
...
RECOMMENDED_LLM_PROVIDERS = {
    "moonshot": (
        "Kimi / Moonshot AI",
        "https://platform.kimi.com?track_id=track-2f5441d6ffd84c509dd079d78e9db5dc&aff=videocraftai",
    ),
    ...
    "volcengine": (
        ...
        "https://www.volcengine.com/activity/ai618?utm_source=VideoCraft AI",
    ),
...
def log(message: str) -> None:
    ...
    print(f"[VideoCraft AI] {message}", flush=True)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install VideoCraft AI and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"VideoCraft AI installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional VideoCraft AI CLI arguments placed after --",
    )
...
def ensure_project(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        archive_path = temp_dir / "VideoCraft AI.zip"
        request = urllib.request.Request(
            PROJECT_ARCHIVE_URL,
            headers={"User-Agent": "VideoCraft AI-Agent-Skill"},
        )
        ...
        if len(candidates) != 1:
            raise SkillError("download completed but no valid VideoCraft AI project was found")
        if root.exists():
            root.rmdir()
...
def _validate_pexels_key(api_key: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "VideoCraft AI-Agent-Skill",
        },
    )
    try:
...
def result_manifest_path(root: Path) -> Path:
    return root / ".agent-logs" / "videocraftai-video" / "latest-result.json"
...
def generate_video(
    ...
    log_dir = root / ".agent-logs" / "videocraftai-video"
