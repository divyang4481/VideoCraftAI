...
# If you cannot download the model from the official website, you can use a mirror website.
# Just remove the comment on the line below.
...
# 0.0.0.0 can only mean "listen on all network cards" and is not suitable as a browser access address.
# When opening http://0.0.0.0:8501 in a browser under macOS/Linux, it may go through a proxy or gateway.
# Eventually 502 appears. Binds and opens 127.0.0.1 by default, consistent with Windows startup scripts.
...
# Use Python for port detection to avoid relying on differences in lsof/nc on different macOS/Linux distributions.
